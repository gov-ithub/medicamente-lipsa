...
@section('content-header')
    <h1>
        {{ trans('meds::patients.title.create patient') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.meds.patient.index') }}">{{ trans('meds::patients.title.patients') }}</a></li>
        <li class="active">{{ trans('meds::patients.title.create patient') }}</li>
    </ol>
@stop
...
@section('content')
    {!! Form::open(['route' => ['admin.meds.patient.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('meds::admin.patients.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
